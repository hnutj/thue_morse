...
Rank1(int n)
{
	...
	if ( isZero(N) ) {
		...
		//32x32的矩阵，每个int类型对应4x8=32个bit，我们只需用32个int即可表示32x32的矩阵
		unsigned int matrix1[32];
		unsigned char* temp = byteEpsilon;
		for ( k = 0; k < N; k++) {
			//字节序有点问题，但矩阵任意几列调换位置不影响秩的求解
			memcpy(matrix1, temp, 128);
			temp += 128;
