...
Universal1(int n)
{
	...
	for (int i = 0; i < K + Q; i++) {
		...
		if (k >= 0) {
			// 涉及1byte
			temp = (byteEpsilon[byteNum] >> k) & mask;
		}
		else if (k >= -8) {
			//涉及2bytes
			temp = ((byteEpsilon[byteNum] << (-k)) | (byteEpsilon[byteNum + 1] >> (8 + k))) & mask;
		}
		else if (k >= -24) {
			//涉及3bytes
			temp = (htonl(((int*)(byteEpsilon + byteNum))[0]) >> (24 + k)) & mask;
		}

		if (i >= Q) {
			sum += log(i + 1 - T[temp]) / log(2);
		}
		T[temp] = i + 1;
	}
