...
//求32×32的0-1矩阵的秩，每32个bit用一个int表示
int Mrank(unsigned int M[32]) {
	...
	for (row = 0; row < 32; row++)
	{
		i = row;
		/*
		若(column_value & M[i])=0，代表矩阵这一行在对应的列上取值为0，我们让指针i往下一行寻找
		直到找到某一行在这一列上不为0(为1)，就交换，若全部行在这一个列都为0，rank-1，索引下一列
		*/
		while (column_value)
		{
			if (column_value & M[i])
