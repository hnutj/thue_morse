...
extern void cufft(int n, double* input, double* output);
...
// void
// DiscreteFourierTransform1(int n)
// {
// 	double	p_value, upperBound, percentile, N_l, N_o, d, *m = NULL, *X = NULL, *X1 = NULL, *wsave = NULL;
// 	int		i, count;
	
// 	if ( ((X = (double*) calloc(n,sizeof(double))) == NULL) ||
// 		 ((X1 = (double*) calloc(n,sizeof(double))) == NULL) ||
// 		 ((wsave = (double *)calloc(2*n,sizeof(double))) == NULL) ||
// 		 ((m = (double*)calloc(n/2+1, sizeof(double))) == NULL) ) {
// 			fprintf(stats[7],"\t\tUnable to allocate working arrays for the DFT.\n");
// 			if( X != NULL )
...
// 	fftw_plan p = fftw_plan_r2r_1d(n, X, X, FFTW_R2HC, FFTW_ESTIMATE);
// 	fftw_execute(p); 
// 	fftw_destroy_plan(p);
...
// 	m[0] = sqrt(X1[0]*X1[0]);	    /* COMPUTE MAGNITUDE */	
// 	for ( i=0; i<n/2; i++ )
// 		m[i+1] = sqrt(pow(X1[2*i+1],2)+pow(X1[2*i+2],2)); 
