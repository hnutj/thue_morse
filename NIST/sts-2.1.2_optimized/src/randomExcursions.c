	...
	for ( i = 0; i < byteNum; i++) {
		...
		for ( j = 0; j < 8; j++) {
			...
			if (S_k == 0) {
				J++;
				for (int i = 0; i < 8; i++) {
					// 第i个状态，在这个循环中出现了count[i]次
					if (counter[i] <= 4) nu[counter[i]][i]++;
					else nu[5][i]++;
					counter[i] = 0;
				}
			}
			// 仍然处在当前循环
				...
				if (S_k < 0 && S_k >= -4) counter[S_k + 4]++;
				else if (S_k > 0 && S_k <= 4) counter[S_k + 3]++;
			}
		}
	}
	// 处理尾巴
